...
//			System.out.println(file.getPath()); // 경로명 + 파일명
			...
			// 자동으로 변환 windows: \\ , 우분투(리눅스): /
			// => 우분투 => war => 톰캣에 올려서 실행
			/*
			 *   C:\webDev\webStudy\.metadata\.plugins\org.eclipse.wst.server.core\tmp0\wtpwebapps\MVCLastProject\WEB-INF\classes
			 *   
			 */
...
//			System.out.println(path);
			...
			// root태그 읽기 => xml (root 태그는 table과 같은 역할)
...
//			System.out.println(pack);
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// http://localhost/MVCLastProject/main/main.do
		//                 ---------------------------- uri
		//                  -------------- ContextPath
				...
				for(Method m:methods) {
					...
					if(rm.value().equals(uri)) {
						...
						if(jsp==null) {
							...
							// sendRedirect : request 초기화 => 화면 변경
							jsp=jsp.substring(jsp.indexOf(":")+1);
							response.sendRedirect(jsp);
							// return "redirect:list.do"
							
						}else {
							// forward : request 유지 => 화면 변경
