	...
	axios.get('music.do')
		.then(response=>{
			...
			list.forEach((m)=>{
				let s=''
				if(m.state==='상승'){
					s='<font color="red">▲</font> '+m.idcrement
				}
				else if(m.state==='하강'){
					s='<font color="blue">▼</font> '+m.idcrement
				}else{
					s='-'
				}
				html+='<tr>'
		...
		key.addEventListener('input',function(){
			...
			const find=document.querySelectorAll('#user-table > tbody > tr > td:nth-child(5n+3):contains("'+key+'")')
