...
<%--
	...
	1. 필요한 이유
	  1) HTTP는 원래 기억을 못하는 구조
	     예) 로그인 => 다음 화면으로 이동 => 로그인 기록을 기억 못함
	  2) 사용자 기억 장치가 필요
	     => Cookie / Session
	     => 상태 유지
	2. Cookie
	 ...
	 3. session : 내장 객체 => 미리 생성된 객체
	    ...
	    4) 기간 설정
	       session.setMaxactiveInterval(초단위)
	                        ==> 기본값 : 1800 => 30분
	    5) 생성된 시점
	       session.getCreationTime() : 세선에 등록된 시간
